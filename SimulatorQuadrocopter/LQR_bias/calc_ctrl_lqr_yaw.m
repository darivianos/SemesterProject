...
R = diag(15);           % command (pitch_cmd)
u_max = MAX_YAW_RATE;
delay = Td_yaw_1stOrd; % in seconds
...
X_ref=[zeros(1,round(1/Ts)) , pi*ones(1,num_of_steps+delay_steps)];
...
for i=1+delay_steps:num_of_steps+delay_steps
    ...
    uu=-K_lqr_yaw*([x_state(1,i)-X_ref(i);x_state(2:n,i)]);
...
PATH_CTRL = ['../Hquad_CTRL_OBSV_parameters/ctrl_lqr_yaw_v'...
                    num2str(VERSION_NR) '.mat'];
save(PATH_CTRL, 'K_lqr_yaw')
