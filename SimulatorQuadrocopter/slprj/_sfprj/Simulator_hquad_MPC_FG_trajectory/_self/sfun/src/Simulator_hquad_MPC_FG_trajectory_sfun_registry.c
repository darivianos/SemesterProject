...
{
  extern unsigned int
    sf_Simulator_hquad_MPC_FG_trajectory_process_check_sum_call( int nlhs,
  ...
  extern unsigned int sf_Hquad_control_LIB_MPC_process_check_sum_call( int nlhs,
    mxArray * plhs[], int nrhs, const mxArray * prhs[] );
  if (sf_Simulator_hquad_MPC_FG_trajectory_process_check_sum_call(nlhs,plhs,nrhs,
       prhs))
    return 1;
...
{
  extern unsigned int sf_Simulator_hquad_MPC_FG_trajectory_autoinheritance_info(
  ...
  if (strcmp(machineName, "Simulator_hquad_MPC_FG_trajectory") == 0) {
    const mxArray *newRhs[3] = { NULL, NULL, NULL };

    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    newRhs[2] = prhs[3];
    return sf_Simulator_hquad_MPC_FG_trajectory_autoinheritance_info(nlhs,plhs,3,
      newRhs);
  }
...
{
  extern unsigned int
    sf_Simulator_hquad_MPC_FG_trajectory_get_eml_resolved_functions_info( int
  ...
  if (strcmp(machineName, "Simulator_hquad_MPC_FG_trajectory") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_Simulator_hquad_MPC_FG_trajectory_get_eml_resolved_functions_info
      (nlhs,plhs,2,newRhs);
  }
...
extern unsigned int sf_Simulator_hquad_MPC_FG_trajectory_method_dispatcher
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"Simulator_hquad_MPC_FG_trajectory")) {
    return(sf_Simulator_hquad_MPC_FG_trajectory_method_dispatcher(simstructPtr,
...
extern void Simulator_hquad_MPC_FG_trajectory_terminator(void);
...
void sf_machine_global_terminator(void)
{
  if (sfMachineGlobalTerminatorCallable) {
    ...
    Simulator_hquad_MPC_FG_trajectory_terminator();
...
extern void Simulator_hquad_MPC_FG_trajectory_initializer(void);
extern void Hqaud_model_LIB_SI_initializer(void);
extern void Hquad_control_LIB_MPC_initializer(void);
extern void Simulator_hquad_MPC_FG_trajectory_register_exported_symbols
  (SimStruct* S);
...
extern void Simulator_hquad_MPC_FG_trajectory_debug_initialize(void);
...
{
  Simulator_hquad_MPC_FG_trajectory_register_exported_symbols(S);
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfMachineGlobalInitializerCallable) {
    ...
    if (!simModeIsRTWGen) {
      Simulator_hquad_MPC_FG_trajectory_debug_initialize();
    }

    Simulator_hquad_MPC_FG_trajectory_initializer();
    if (!simModeIsRTWGen) {
