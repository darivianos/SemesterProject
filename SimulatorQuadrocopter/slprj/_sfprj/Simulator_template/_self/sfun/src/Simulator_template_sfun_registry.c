  ...
  nrhs, const mxArray * prhs[] )
{
  ...
  if (strcmp(machineName, "Simulator_template") == 0) {
  ...
  plhs[], int nrhs, const mxArray * prhs[] )
{
  ...
  if (strcmp(machineName, "Simulator_template") == 0) {
    ...
    newRhs[0] = prhs[0];
    newRhs[1] = prhs[2];
    return sf_Simulator_template_get_eml_resolved_functions_info(nlhs,plhs,2,
      newRhs);
  }
...
unsigned int sf_machine_global_method_dispatcher(SimStruct *simstructPtr, const
  ...
  method, void *data)
{
  if (!strcmp(machineName,"Simulator_template")) {
    return(sf_Simulator_template_method_dispatcher(simstructPtr,chartFileNumber,
...
void sf_machine_global_initializer(SimStruct* S)
{
  ...
  if (sfGlobalMdlStartCallCounts == 1) {
    ...
    if (!simModeIsRTWGen) {
      Simulator_template_debug_initialize(sfGlobalDebugInstanceStruct);
    }
