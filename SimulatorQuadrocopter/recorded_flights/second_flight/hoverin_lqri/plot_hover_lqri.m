...
load hquad_pos_lqri.mat

 plot([hquad_pos_lqri(2,:)',...
    hquad_pos_lqri(6,:)']);
...
end_time_camera=start_time_camera+23/Ts; %4612


% move starting point to 0/0
hquad_pos_lqri(2,:)=hquad_pos_lqri(2,:)-2;
hquad_pos_lqri(5,:)=hquad_pos_lqri(5,:)-2;
hquad_pos_lqri(7,:)=hquad_pos_lqri(7,:)+0.4;
hquad_pos_lqri(10,:)=hquad_pos_lqri(10,:)+0.4;
hquad_pos_lqri(12,:)=hquad_pos_lqri(12,:)+1;
hquad_pos_lqri(14,:)=hquad_pos_lqri(14,:)+1;

% data is recorded with z-axis pointing down !!
% change this
hquad_pos_lqri(7:19,:)=-1*hquad_pos_lqri(7:19,:);

% get trajectory data
hovering_vicon=hquad_pos_lqri(:,start_time:end_time);
hovering_camera=hquad_pos_lqri(:,start_time_camera:end_time_camera);

plot_nicely(hquad_pos_lqri,start_time_camera,end_time_camera,'Hovering with Camera')

% %% x - axis
% figure
% plot(hovering_vicon(1,:)'-hovering_vicon(1,1),hovering_vicon(2,:)','b',...
%     hovering_vicon(1,:)'-hovering_vicon(1,1),hovering_vicon(5,:)','r');
...
% %% yaw - angle
% % figure
% % plot(hquad_pos_lqri(1,:)'-hquad_pos_lqri(1,1),[hquad_pos_lqri(16,:)',...
% %     hquad_pos_lqri(17,:)',hquad_pos_lqri(18,:)']);
...
% % %% 2D - Plot
% % figure
% % plot(hquad_pos_lqri(2,:)', hquad_pos_lqri(7,:)');
% % hold on
% % plot(hquad_pos_lqri(5,:)', hquad_pos_lqri(10,:)','-r');
...
% % %% 3D - Plot
% % figure
% % plot3(hquad_pos_lqri(2,:)', hquad_pos_lqri(7,:)',hquad_pos_lqri(12,:)');
% % hold on
% % plot3(hquad_pos_lqri(5,:)', hquad_pos_lqri(10,:)',hquad_pos_lqri(14,:)','-r');
